...
typedef union{
    uint8_t kp[4];
    float Kp;       // 与4字节结构体共享内存
}KP_params;
typedef union{
    uint8_t ki[4];
    float Ki;       // 与4字节结构体共享内存
}KI_params;
typedef union{
    uint8_t kd[4];
    float Kd;       // 与4字节结构体共享内存
}KD_params;
typedef union{
    uint8_t kp[4];
    uint32_t Kp;       // 与4字节结构体共享内存
}KP_INT_params;
typedef union{
    uint8_t ki[4];
    uint32_t Ki;       // 与4字节结构体共享内存
}KI_INT_params;
typedef union{
    uint8_t kd[4];
    uint32_t Kd;       // 与4字节结构体共享内存
}KD_INT_params;

extern KP_params KP_data;
extern KI_params KI_data;
extern KD_params KD_data;
extern KP_INT_params KP_INT_data;
extern KI_INT_params KI_INT_data;
extern KD_INT_params KD_INT_data;
