...
void delay_us(uint32_t nus)
{
		...
		tCnt = 0;
		tDelayCnt = nus * (SystemCoreClock / 1000000);
...
void delay_125_ns(uint32_t nns)
{
	#if DWT_Delay 
		uint32_t tCnt, tDelayCnt;
		uint32_t tStart;
		...
		tStart = DWT_CYCCNT; /* 刚进入时的计数器值 */
		tCnt = 0;
		tDelayCnt = nns * (SystemCoreClock / 1000000)/8;//1us的八分之一的计数为125ns
		/* 需要的节拍数 */    /*SystemCoreClock :系统时钟频率*/                 
...
void delay_14_ns(uint32_t nns)
{
		...
		tStart = DWT_CYCCNT; /* 刚进入时的计数器值 */
		tCnt = 0;
		tDelayCnt = nns * (SystemCoreClock / 1000000)/72;//1us的72分之一的计数为13.88ns约为14ns
		/* 需要的节拍数 */    /*SystemCoreClock :系统时钟频率*/                 
